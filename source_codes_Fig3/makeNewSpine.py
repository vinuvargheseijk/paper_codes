...
class essential_lists():
    ...
    def content(self, pool_name, summation_vecs):
        ...
        for i in range(len(summation_vecs)):
            ...
            print("Effective conc: ", ( 1e3 * sum(moose.vec('/model/chem/dend/cytosol').n) + temp_count ) / (TE.V_cyt * TE.Na))
...
def check_vicinity_for_threshold(search_radius, threshold_value):
      searchLimits = [2e-6, search_radius - 2e-6] #Buffer zone of 2 um near boundary
      searchLimits_voxels = [searchLimits[0] / TE.diffL, searchLimits[1] / TE.diffL]
      update_max_value = threshold_value
      thr_flag = 0
      max_index = int( (searchLimits_voxels[1] - searchLimits_voxels[0]) / 2.0 )
      cross_threshold_indices = []
      cross_threshold_values = []
      ...
      for sr in range(int(lower_search_limit), int(upper_search_limit)):
             ...
             ###The following looks for the maximum value voxel
             print("Threshold flag: check_vicinity: ", thr_flag)
             if thr_flag == 1 and loc_value >= update_max_value:
                     update_max_value = loc_value
                     max_index = sr
                     if update_max_value > threshold_value:
                          print("Recording threshold values")
                          cross_threshold_indices.append(max_index)
                          cross_threshold_values.append(update_max_value)
      print("Reached threshold max loc: ", max_index * TE.diffL)
      print("Threshold indices: ", cross_threshold_indices)
      print("Threshold values: ", cross_threshold_values)
      print("Max conc: ", update_max_value)
      if len(cross_threshold_values) > 0:
         thr_flag = 1
      print("Threshold flag return: check_vicinity: ", thr_flag)
      return thr_flag, max_index * TE.diffL, np.asarray(cross_threshold_indices) * TE.diffL       
...
def check_proximity(location, threshold_locations, tolerance):
    pr_flag = -1
    print("Checking proximity: ", location)
    print("UPDATED LOCATIONS in check_proximity: ", updated_locations)
    diff_distances = []
    preferred_locations = []
    for tlocs in threshold_locations:
        print("Threshold loc preferred search: ", tlocs)
        for u_l in updated_locations:
            if abs(tlocs - u_l) < tolerance:
               add_loc = False
               break
            else:
               add_loc = True
        if add_loc:
           diff_distances.append(abs(location - tlocs))
           preferred_locations.append(tlocs)
    if len(diff_distances) > 0:
       min_l = diff_distances.index(min(diff_distances))
       updated_location = preferred_locations[min_l]
    else:
    ...
    if len(preferred_locations) > 0:
       ...
       pr_flag = 0
    return pr_flag, updated_location



dt_list = []
if tag.chemTurnOff == False:
   stim_start_time = tag.stim_start_time
   stim_end_time = tag.stim_end_time   
else:
   stim_start_time = 0
...
plot_interval = 2
init_dummy = tag.init_dummy

dummy_spine_locations = tag.dummy_spine_locations

print("DUMMY SPINE LOCATIONS: ", dummy_spine_locations)

def init_dummy_create(n):
      print("Creating spine")
      new_spine_attrib(dummy_spine_locations[n], 0.5e-6, 0.2, dummy)
      updated_locations.append(dummy_spine_locations[n])

delete_spine_flag = True
if tag.chemTurnOff == False:
...
spine_create_delay = tag.spine_create_delay
spine_create_times = tag.spine_create_times

def physics_spine_create(current_time):
     if current_time > spine_create_times[0]:
          print("Physics only spine creating")
          print(spine_create_times[0], current_time)
          init_dummy_create( 0 )
          dummy_spine_locations.pop(0)
          spine_create_times.pop(0)

if init_dummy:
   dummy_create_flag = True
...
while current_time < runtime:
   if tag.chemTurnOff:
      if len(spine_create_times) > 0:
         physics_spine_create(current_time)
   opt_sample_min = 10
   ...
   def spec_loc_create():
     if cytosol_conc > threshold_val and dummy_create_flag:
         print("Entering spine creating part")
         if len(dummy_spine_locations) > 0:
           if current_time > spine_create_times[0]:
              print("CREATING DUMMY SPINE")
              print("CYTOSOL CONC: ", cytosol_conc)
              if len(updated_locations) < num_spines:
                init_dummy_create( 0 )
                dummy_spine_locations.pop(0)
                spine_create_times.pop(0)
              else:
                dummy_create_flag = False
     print("to be created: ", dummy_spine_locations)


   def search_opt_loc(new_spine_created_time):
     if len(updated_locations) < num_spines:
       for scl in range(len(updated_locations), num_spines):
         print("Nth spine: ", scl)
         print("CHECKING THRESHOLD")
         print("Threshold: ", threshold_val)
         threshold_flag, loc_update, all_threshold_locs = check_vicinity_for_threshold(tag.Length * 1e-6, threshold_val)
         print("Threshold_flag: ", threshold_flag, loc_update)
         if threshold_flag == 1:
           if len(updated_locations) > 0:
                 pr_flag, next_close_loc = check_proximity(loc_update, all_threshold_locs, 0.7e-6)
           else:
                 pr_flag = 1
                 next_close_loc = loc_update
           print("Proximity flag: ", pr_flag)
           print("Max and possible: ", loc_update, next_close_loc)
           dflag = check_in_domain(next_close_loc)
           print("In domain flag: ", dflag)
           if dflag == 1:
               if pr_flag == 1 or pr_flag == -1:
                    print("CREATING SPINE")
                    new_spine_attrib(next_close_loc, 0.5e-6, 0.2, dummy)
                    new_spine_created_time = current_time
                    updated_locations.append(next_close_loc)
                    break
               else:
       ...
       return new_spine_created_time
   if tag.chemTurnOff == False:
      if (current_time - new_spine_created_time) > 1:
   ...
   if len(updated_locations) > 0 and (current_time - last_update_time) > shape_dt:
      #if current_time % shape_dt == 0: 
         print("Updating shape at: ,", current_time)
         s = essential_lists()
         mgd.magic_diff(s.dome_start_voxels, s.dome_end_voxels)
         last_update_time = current_time
         if current_time - deletion_time > 1 and tag.chemTurnOff == False:
               ...
               voxel_wise_concs = 1e3 * np.asarray(Tiam_vec) / (TE.Na * np.pi * TE.Rc**2 * TE.diffL)
               min_voxel_conc = min(voxel_wise_concs)
               print("Min conc Tiam: ", min_voxel_conc)
               for scrd in range(len(s.dome_start_voxels)):
                   ...
                   concs_Tiam = 1e3 * num_mols_Tiam / (TE.Na * np.pi * TE.Rc**2 * TE.diffL * num_voxels_Tiam)
                   #concs_Tiam = 1e3 * num_mols_Tiam / (TE.Na * TE.V_cyt)
                   conc_fractions.append(concs_Tiam / 2.0)  #2 uM is the conc of Tiam_inact
                   print("Conc Tiam: ", concs_Tiam)

               print("Effective concs dend: ", effective_concs_dend)
               location_under_threshold =  np.where(np.asarray(effective_concs_dend) < threshold_val * 1e3)[0]
               random_elimination_probability = np.random.uniform(0.0, 1.0, len(s.dome_start_voxels))
               tiam_dependent_probability = tag.TK * np.asarray(conc_fractions)
               deletion_pairs = []
               for lut in location_under_threshold:
                   if conc_fractions[lut] < 0.2:
                     deletion_pairs.append( [ lut, random_elimination_probability[lut], tiam_dependent_probability[lut] ] )
               print("Locations under Threshold: ", location_under_threshold)
               print("Random prob: ", random_elimination_probability)
               print("Tiam dependent prob: ", tiam_dependent_probability)
               if len(deletion_pairs) > 0:
                 print("Deletion list: ", deletion_pairs)
                 max_prob = 0.0
                 max_prob_index = 0
                 deletion_prob_flag = False
                 for dp in range(len(deletion_pairs)):
                       if deletion_pairs[dp][1] > max_prob:
                          max_prob = deletion_pairs[dp][1]
                          max_prob_index = dp
                          deletion_prob_flag = True
                 print("Max prob at: ", max_prob, max_prob_index)
                 if tag.tiam_dependency:
                    if deletion_pairs[max_prob_index][1] >= deletion_pairs[max_prob_index][2] and deletion_prob_flag:
                        spine_for_deletion = deletion_pairs[max_prob_index][0]
                        rand_prob_list.append( deletion_pairs[max_prob_index][1] )
                        rand_prob_spines.append( spine_for_deletion )
                        print("Spine for deletion: ", spine_for_deletion)
                        print("Conc Frac: ", conc_fractions)
                        print("Conc Frac for deletion falgged: ", conc_fractions[spine_for_deletion])
                        print("Deleting spine: ", current_time, spine_for_deletion)
                        ...
                        if deletion_prob_flag and deletion_pairs[max_prob_index][1] >= 0.5:
                           spine_for_deletion = deletion_pairs[max_prob_index][0]
                           rand_prob_list.append( deletion_pairs[max_prob_index][1] )
                           rand_prob_spines.append( spine_for_deletion )
                           print("(Tiam independent) Spine for deletion: ", spine_for_deletion)
                           print("Conc Frac: ", conc_fractions)
                           print("Conc Frac for deletion falgged: ", conc_fractions[spine_for_deletion])
                           print("Deleting spine: ", current_time, spine_for_deletion)
                           delete_voxels = list(np.arange(s.dome_start_voxels[spine_for_deletion] - 10, s.dome_end_voxels[spine_for_deletion] + 10,1))
   ...
   print("Saving molecules values at: ", current_time)          
   ...
   current_time = current_time + plot_save_dt
