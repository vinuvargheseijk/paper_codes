...
def shift_time_exp(time_exp, shift):
    time_exp = np.asarray(time_exp) + shift
    return time_exp
...
V_cyt = np.pi * (0.5 * TE.dendDia)**2 * TE.Length * 1e-6
dt = 0.2
plot_interval = 1
t_axis = np.linspace(0, tag.runtime, int(tag.runtime/dt))
#df_exp = pd.read_csv("./Hedrick_Rac1_2016.csv")
#print(df_exp)
tag_string = tag.tag_string
memXML = tag_string + "membrane.xml"
RacXML = tag_string + "Tiam.xml"
cytosolXML = tag_string + "cytosol.xml"
CaXML = tag_string + "Ca.xml"
check_files = [memXML, RacXML, cytosolXML, CaXML]
names = ["Mem", "Tiam", "cytosol", "Ca"]
chemTimes = pd.read_csv(tag_string + "chemTimes.csv")["time"]
print(chemTimes)
mol_name = 0
#norm_exp = normalize_exp_sim(np.asarray(df_exp["conc"]) - min(list(df_exp["conc"])))
#time_exp = df_exp["time"]
shift_time = 0.5 #simulation pulse comes at 0.5 s
#time_exp = shift_time_exp(time_exp, shift_time)
#print(time_exp)
#ax1.plot(np.asarray(time_exp), norm_exp, label = "Exp.")
for c in check_files:
     filename = c
     tree1 = ET.parse(filename)
     tree = [tree1]
     sum_an,max_an_y,an_y=RX.plotXML(filename,tree)
     if names[mol_name] == "Tiam":
         #ax1.plot( t_axis, normalize_exp_sim(max_an_y), 'r', label = names[mol_name] )
         ax1.plot(chemTimes, 1e3 * np.asarray(max_an_y) / (V_voxel * TE.Na) , 'r', label = names[mol_name] )
         #norm_Rac = normalize_exp_sim(np.asarray(max_an_y)- min(max_an_y))
         #ax1.plot( t_axis, norm_Rac, 'b', label = names[mol_name] ) #Actual conc.
     if names[mol_name] == "Mem":
         #ax2.plot( t_axis, normalize_exp_sim(np.asarray(max_an_y) - min(max_an_y)), 'b', label = names[mol_name] )
         ax2.plot(chemTimes,  1e3 *  ( np.asarray(max_an_y) - min(max_an_y) ) / (V_voxel * TE.Na), 'b', label = names[mol_name] ) #Actual conc.
     ...
     if names[mol_name] == "cytosol":
         ...
         ax3.plot(chemTimes,  1e3 * np.asarray(sum_an) / (V_cyt * TE.Na), 'g', label = names[mol_name] )
     if names[mol_name] == "Ca":
         ax3.plot(chemTimes, 1e3 * np.asarray(sum_an) / (V_cyt * TE.Na), 'b', label = names[mol_name] )
     #    ax3.plot(  np.asarray(sum_an) / (V_voxel * TE.Na)  , 'c', label = names[mol_name] )
     mol_name = mol_name + 1
