...
def get_parameters_merge(conc, spacing, spine_index):
   df = pd.read_csv("./CytConc_" + str(conc) + "_ns_2_spine_create_delay_25.0_spine_spacing_" + str(spacing) + "hmean.csv")
   print("./CytConc_" + str(conc) + "_ns_2_spine_create_delay_25.0_spine_spacing_" + str(spacing) + "hmean.csv")
   print(df)
   for i in range(len(df)):
...
def get_parameters_select_time(conc, spacing, time, spine_index):
   df = pd.read_csv("./CytConc_" + str(conc) + "_ns_2_spine_create_delay_25.0_spine_spacing_" + str(spacing) + "hmean.csv")
   print("./CytConc_" + str(conc) + "_ns_2_spine_create_delay_25.0_spine_spacing_" + str(spacing) + "hmean.csv")
   df = pd.read_csv("./CytConc_" + str(conc) + "_ns_2_spine_create_delay_25.0_spine_spacing_" + str(spacing) + "shapeTimes.csv")
...
#These are for energy comparisons for N spines
"""
markers = ["X", "o", "v"]
ax1_colors = ["b", "m", "g"]
...
spacing_competition = [0.6, 0.7, 0.8, 1.0, 1.5, 2.0, 2.5]
conc_list = [0.8, 1.0, 1.2]
conc_competition = 1.0
delay = 25.0
ticklabels = True
select_spacings = [0.6, 0.8, 1.5]
...
for cc in conc_list:
  ...
  for sp in spacing_competition:
    ...
    if cc == 1.0 and sp == 1.5:
      x0, y0, time_2_init = get_parameters(cc * 1e-3, sp, 0)
      x1, y1, time_2_init = get_parameters(cc * 1e-3, sp, 1)
      heatmap_inset1 = inset_shapes(heatmap_axis, x0, y0, x1, y1, 0.95, 0.45, "b")
      ref_line(time_2_init, "b", 11.0)


      x0, y0 = get_parameters_select_time(cc * 1e-3, sp, time_2_init + 25, 0)
      x1, y1 = get_parameters_select_time(cc * 1e-3, sp, time_2_init + 25, 1)
      heatmap_inset2 = inset_shapes(heatmap_axis, x0, y0, x1, y1, 0.95, 0.85, "m")
      ref_line(time_2_init + 25, "m", 11.0)
...
heatmap_axis = ax31H
plot_heatmap = True
x0, y0, time_2_merge = get_parameters_merge(cc * 1e-3, sp, 0)
x1, y1, time_2_merge = get_parameters_merge(cc * 1e-3, sp, 1)
heatmap_inset1 = inset_shapes(heatmap_axis, x0, y0, x1, y1, 0.8, 0.58, "b")
ref_line(time_2_merge, "b", 10.6)

x0, y0 = get_parameters_select_time(cc * 1e-3, sp, time_2_merge + 25, 0)
#x1, y1 = get_parameters_select_time(cc * 1e-3, sp, time_2_merge + 10, 1)
heatmap_inset2 = inset_shapes(heatmap_axis, x0, y0, [], [], 0.75, 0.93, "m")
...
ax31H.text(0.0, 0.99, str(cc) + ": $\mu M$," + str(sp) + ": $\mu m$", weight = "normal",
        horizontalalignment='left',
        verticalalignment='bottom',
        transform=ax31H.transAxes)
#heatmap_axis.set_title(str(cc) + ": $\mu M$," + str(sp) + ": $\mu m$")
...
ax32H.set_title("Concentration 2nd spine $\mu M$")
ax32H.set_title("spacing ($\mu m$)")
#ax32H.set_xlim([delay + 1, delay + 2.0])
ax32H.legend(frameon = False)
ax32H.spines[['right', 'top']].set_visible(False)
ax32H.set_xlabel("Spacing $\mu m$")
ax32H.set_ylabel("Local Conc. at spine2 \n $\mu M$")
ax32H.set_title("" )
ax32H.set_title("" )
ax32H.set_title("" )
ax32H.set_ylim([0, 1])
...
spacing = 0.8
conc = 1.4
chemTimes, conc1, conc2, cytConc1, cytConc2 = plot_conc_heatmap(conc, 2, delay, spacing, "membrane", ax31H, 10, 1.5, [0, 80], ticklabels, True, True)
ax31H.set_ylabel("t $(s)$")
ax31H.set_xlabel("x $(\mu m)$")
"""
def plot_exp_data():
...
plt.figure(1).ax_dict["<colorbar>"].set(position=[0.9141, 0.11, 0.0129, 0.1925])
plt.figure(1).axes[1].set(position=[0.4515, 0.6875, 0.1938, 0.1925])
plt.figure(1).axes[2].set(position=[0.7494, 0.6875, 0.1937, 0.1925])
plt.figure(1).axes[4].set(position=[0.4515, 0.3988, 0.1938, 0.1925])
plt.figure(1).axes[5].set(position=[0.7494, 0.3988, 0.1937, 0.1925])
plt.figure(1).axes[7].set(position=[0.4515, 0.11, 0.1937, 0.1925], ylabel='Local Conc. at spine2 $\\mu M$')
plt.figure(1).axes[8].set(position=[0.7494, 0.11, 0.155, 0.1925])
