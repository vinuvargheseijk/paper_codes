...
def min_space(axis):
    ...
    axis.invert_yaxis()
    axis.set_xlabel("Time delay (s)")
    return df


def extract_line_conc(cytConc, N, spine_create_delay, spine_spacing, filename, time_plot):
       ...
       time_index = np.where(chemTimes["time"] > time_plot)[0][0]
       print("Spatial conc for H plotted at: ", time_index, time_plot)
       ...
       return np.asarray(x_axis) - 0.5 * Length * 1e6, 1e3 * np.asarray(an_y[time_index]) / (v_cyt * Na / num_voxels)

def extract_shapes_panelH(cytConc, N, spine_create_delay, spine_spacing, time_plot):
       tag_string = "CytConc_" + str(round(cytConc * 1e-3, 5)) + "_ns_" + str(N) + "_spine_create_delay_" + str(spine_create_delay) + "_spine_spacing_" + str(spine_spacing)
       sstart = pd.read_csv("./" + tag_string + "sstart.csv")
       shapeTimes = pd.read_csv(tag_string + "shapeTimes.csv")
       time_index = np.where(shapeTimes["0"] > time_plot)[0][0]
       saddle_coord = sstart.iloc[time_index]["0"]
       hmean = pd.read_csv(tag_string + "hmean.csv")
       theta = pd.read_csv(tag_string + "theta.csv")
       rp = pd.read_csv(tag_string + "rp.csv")
       rm_v = 1 / hmean.iloc[time_index]["0"]
       theta_v = theta.iloc[time_index]["0"]
       rp_v = rp.iloc[time_index]["0"]
       half_size = (rm_v + rp_v) * np.sin(theta_v) * 1e6
       return half_size
        ...
        for l2 in range(len(list2)):    
             ...
             try:
               ...
               for n in range(spine_number):
                   sum_N_energy = sum_N_energy + EP.total_energy([ last_rd[n] * 1e6 * last_theta[n], last_theta[n], last_rp[n] * 1e6 ], phitot[n], last_conc[n], k_agg, -4.5)
                   sum_ag_energy = sum_ag_energy + EP.aggregation_energy(last_phi[n], phitot[n], area[n], k_agg) / EP.KBT
...
def energy_compare_N():
   ...
   for eccl in energy_compare_conc_list:
       N4E, HRM, HTHETA, HRP, LRM, LTHETA, LRP = NCE.extract_data(-4.5, k_agg, 4, eccl)
       if eccl == select_eccl_shape:
         x, y, dms, dme = dendShape.dendShape( 0, HTHETA, HRM, HRP )
         axin11H_1 = ax11H.inset_axes([4, 4.7, 3.0, 4.0], transform = ax11H.transData) 
         ...
         axin11H_1.set_ylim([0, dend_plot_height + 0.12])
         axin11H_1.set_xlim([4, 5.5])
         axin11H_1.set_xticks([])
         axin11H_1.set_yticks([])
         axin11H_1.set_aspect(1)
       ...
       N2E, HRM, HTHETA, HRP, LRM, LTHETA, LRP = NCE.extract_data(-4.5, k_agg, 2, eccl)
       if eccl == select_eccl_shape:
         x, y, dms, dme = dendShape.dendShape( 0, HTHETA, HRM, HRP )
         axin11H_2 = ax11H.inset_axes([4, 2.1, 3.0, 4.0], transform = ax11H.transData) 
         ...
         for nplot in range(2):
            ...
            plot_offset = plot_offset + 0.6
         #x, y, dms, dme = dendShape.dendShape( 0, LTHETA, LRM, LRP )
         axin11H_2.set_ylim([0, dend_plot_height + 0.12])
         axin11H_2.set_xlim([4, 5.2])
         axin11H_2.set_xticks([])
         axin11H_2.set_yticks([])
         axin11H_2.set_aspect(1)
       ...
       N1E, HRM, HTHETA, HRP, LRM, LTHETA, LRP = NCE.extract_data(-4.5, k_agg, 1, eccl)
       if eccl == select_eccl_shape:
         x, y, dms, dme = dendShape.dendShape( 0, HTHETA, HRM, HRP )
         axin11H_3 = ax11H.inset_axes([4, 0.0, 3.0, 4.0], transform = ax11H.transData) 
         axin11H_3.spines[['right', 'top', 'bottom', 'left']].set_visible(False)
         #axin11H_3.add_patch( Rectangle( (0, 0), 2, dend_plot_height, fc = 'none', ec = 'grey') )
         axin11H_3.plot([4, 4], [0, 0.5])
         plot_offset = 4
         shape_lists_x.append(x)
         shape_lists_y.append(y)
         ...
         axin11H_3.set_ylim([0, dend_plot_height + 0.12])
         axin11H_3.set_xlim([4, 5.0])
         ...
         axin11H_3.plot(x_array, [0] * 2, 'k')  
         axin11H_3.plot([x_array[0]] * 2, [0.0, 0.2], 'k')  
         axin11H_3.text(x_array[-1], 0, "0.2 $\mu m$")
         axin11H_3.text(x_array[0], 0.2, "0.2 $\mu m$")
         axin11H_3.set_aspect(1)
...
ax1H.invert_yaxis()
ax1H.set_title("Final N spines")
ax1H.set_xlabel("IRSp53 ($\mu M$)")
...
ax32H_conc_list = [0.6, 0.8, 1.4]
ax32H_plot_count = 0
time_plot = 50.0
color_list = ["b", "r", "k"]
half_sizes = []
color_count = 0
for c in ax32H_conc_list:  
   x_panelH, cyt_profile_panelH = extract_line_conc(c, 1, 0.0, 0.55, "cytosol", time_plot)
   if ax32H_plot_count == 0:
      ax32H.plot(x_panelH, cyt_profile_panelH, color = color_list[color_count], label = "Total Conc. \n" + str(c) + " $\mu M$")
   else:
      ax32H.plot(x_panelH, cyt_profile_panelH, color = color_list[color_count], label = str(c) + " $\mu M$")
   ax32H_plot_count += 1
   half_size = extract_shapes_panelH(c, 1, 0.0, 0.55, time_plot)
   half_sizes.append(half_size)
   color_count += 1
   print("Half size: ", half_size)
Rectangle1 = Rectangle((0, 0.2),half_sizes[0], 0.025, fc = color_list[0], ec = None,lw = 0.0, hatch = '///')
Rectangle2 = Rectangle((0, 0.25),half_sizes[1], 0.025, fc = color_list[1], ec = None,lw = 0.0, hatch = '///')
Rectangle3 = Rectangle((0, 0.3),half_sizes[2], 0.025, fc = color_list[2], ec = None,lw = 0.0, hatch = '///')
#Rectangle4 = Rectangle((0, 0.8),half_sizes[3], 0.4, fc = None, ec = color_list[3],lw = 0.1, hatch = '///')
ax32H.add_patch(Rectangle1)
ax32H.add_patch(Rectangle2)
ax32H.add_patch(Rectangle3)
ax32H.set_ylabel("IRSp53 conc \n $\mu M$")
ax32H.set_xlabel("x $\mu m$ \n from the spine")
ax32H.set_xlim(0, 10)
ax32H.set_ylim(0, 0.5)
ax32H.set_yticks([0, 0.2, 0.4])
ax22H.set_ylabel("")
ax22H.set_yticks([])
ax23H.set_ylabel("")
ax23H.set_yticks([])
ax32H.legend(frameon = False)
ax32H.legend(loc = "upper right")
...
heatmap_inset1 = inset_shapes(heatmap_axis, x0, y0, x1, y1, 1.0, 0.75, "b")
ref_line(time_2_merge, "b", 11.0)

x0, y0 = get_parameters_select_time(cc * 1e-3, sp, time_2_merge + 5, 0, delay)
#x1, y1 = get_parameters_select_time(cc * 1e-3, sp, time_2_merge + 10, 1)
heatmap_inset2 = inset_shapes(heatmap_axis, x0, y0, [], [], 0.8, 0.85, "m")
ref_line(time_2_merge + 5.0, "m", 10.6)
...
mem_plot(df_conc, time_2_merge + 5, heatmap_inset2)
...
plt.subplots_adjust(left=0.1, bottom=None, right=0.95, top=None, wspace=0.8, hspace=0.8)
#% start: automatic generated code from pylustrator
plt.figure(1).ax_dict = {ax.get_label(): ax for ax in plt.figure(1).axes}
import matplotlib as mpl
getattr(plt.figure(1), '_pylustrator_init', lambda: ...)()
plt.figure(1).axes[1].set(position=[0.392, 0.6945, 0.1848, 0.1674])
plt.figure(1).axes[1].texts[0].set(position=(0.0197, 1.208))
plt.figure(1).axes[2].legend(loc=(0.7309, 0.6326), frameon=False)
plt.figure(1).axes[7].legend(loc=(0.3108, 0.0737), frameon=False, fontsize=24.)
#% end: automatic generated code from pylustrator
plt.show()
