...
print("Parameters: ", k_agg, k_agg2, phi_entire)
...
#For stability check with phi_entire
##########################################
if tag.fix_conc == True:
      moose.vec('/model/chem/dend/cytosol').n = num_calc(phi_entire)
      print(moose.vec('/model/chem/dend/cytosol').n)
      print(moose.vec('/model/chem/dend/cytosol').conc)
else:
     molecules_per_voxel = phi_entire / num_voxels
     moose.vec('/model/chem/dend/cytosol').n = molecules_per_voxel
     volume_vec = calc_volume_voxel(num_voxels)
def cytosol_conc_init(phitot_init):
    cyt_deduction = phitot_init / num_voxels
    print("Cytosol before deduction: ", sum(moose.vec('/model/chem/dend/cytosol').n))
    moose.vec('/model/chem/dend/cytosol').n = np.asarray(moose.vec('/model/chem/dend/cytosol').n) - cyt_deduction
    balance = sum( moose.vec('/model/chem/dend/cytosol').n )
    print("Cytosol after deduction: ", balance)
    initial_cyt_conc = balance / (TE.Na * TE.V_cyt)
    return initial_cyt_conc
##########################################
scalF = tag.scalF
init_phitot = distribute_mols(scalF, tag.initial_phitot)
"""
for ns in range(num_spines):
   dummy = {True : 0, 'phitot' : init_phitot[ns]}
   stim_with_spine(stim_locations[0], 0, 1, 0.1e-12, mem_coords, 0, dummy)
"""   
##########################################
print("SUM OF phitots and initial phitot", sum(init_phitot), tag.initial_phitot)

#perturb schedule
p_schedule =  []
p_code = tag.p_code
perturb_num = tag.perturb_num
# code 1: [ [1, 2, 500], [3, 4, 600], [1, 5, 700] ]
# code 2: [ [1, 500], [2, 600], [3, 700], [4, 800], [5, 900] ]

spine_creation_count = 0
for r in range( int(runtime/dt) ):
   current_time = r * dt
   if len(stim_time_ItNum) > 0:
     print(stim_time_ItNum)
     global sim_count_it
     sim_count_it = len(stim_time_ItNum)
     for itsp in range(sim_count_it):
       if  r >= stim_time_ItNum[0]:
         if num_spines > 1:  
            print("Creating spine at", r * dt) 
            #dummy = {True : 0, 'phitot' : init_phitot[spine_creation_count]}  #Careful to change the phitot when dummy[True] = False because then it is a mature spine with molecules inside it.
            ...
            initial_cyt_conc = cytosol_conc_init(dummy['phitot']) 
         if num_spines == 1:   
            #dummy = {True : 0, 'phitot' : tag.initial_phitot}  #Careful to change the phitot when dummy[True] = False because then it is a mature spine with molecules inside it.
            dummy = {True : 1, 'phitot' : 1.0}  #Careful to change the phitot when dummy[True] = False because then it is a mature spine with molecules inside it.
            initial_cyt_conc = cytosol_conc_init(dummy['phitot']) 
         stim_with_spine(stim_locations[0], 0, 1, 0.1e-12, mem_coords, current_time, dummy)
         sim_count_it = len(stim_time_ItNum)
         print("Stim list: ", stim_time_ItNum)
         spine_creation_count = spine_creation_count + 1
   ...
   if len(location_list) > 0:
      ...
      try:
         ...
         if p_schedule[0][-1] / dt == r:
           for sp in range(len(p_schedule[0][0:-1])):
              perturb_spine(p_schedule[0][sp], s.dome_start_voxels, s.dome_end_voxels, perturb_num)
           p_schedule.pop(0)
