...
class data_extract:
  ...
  def data_save_lists():
    ...
    x_axis = np.linspace(0, len(phitot1_all) * dt * plot_interval, len(phitot1_all)) 
    ...
    for ns in range(1, 10):
     x_axis = np.linspace(0, len(globals()["phitot" + str(ns) + "_all"]) * dt * plot_interval, len(globals()["phitot" + str(ns) + "_all"])) 
    ...
    if conserv_check == True:   
       ...
       ax10.plot( sum_mols, label = "Total" )
       ax10.set_title( " Conservation Check") 
       ax10.legend()
       """
       ax71.legend()
       """
