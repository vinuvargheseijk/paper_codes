...
stimfile = pd.read_csv("./" + tag.tag_string + "stimD.csv")
...
texts_in=ax3.text(.2,-0.6, "{}".format(0.1), transform=ax3.transAxes, color = 'k')
#texts=ax2.text(.1,-0.7, "{}".format(0.1), transform=ax1.transAxes, color = 'b')
#texts_cyt=ax3.text(.1,-0.8, "{}".format(0.1), transform=ax1.transAxes, color = 'r')
#texts_cytC=ax3.text(.1,-0.9, "{}".format(0.1), transform=ax1.transAxes, color = 'g')
...
xmlfilename3 = tag_string+"cytosol.xml"
xmlfilename4 = tag_string+"Rac.xml"
...
freq_ratio = tag.freq_ratio
dt1 = 1/tag.freq
freq2 = tag.freq * freq_ratio
dt2 = 1/freq2

stimTimes = list(stimfile["stimTimes"])
def closest(time):
     lst = np.asarray(stimTimes)
     idx = (np.abs(lst - time)).argmin()
     return lst[idx], idx
...
def update(i):
  ...
  try:
      ...
      if time - ct < 0.1:
        circle1 = plt.Circle((stimfile["stimLocations"][it] * 1e6, 4), 0.1, fc = 'r', ec='r')
  ...
  #ax1.clear()
  max_dt = max([dt1, dt2])
  end_stim = tag.num_bursts * max_dt
  #global df1  
  #global df2 
  #c = list(df1.iloc[i][1:])
  ...
  #c.append(c[0])
  #c1.append(c1[0])
  #texts_in.set_text('Rac')
  #texts.set_text("thr286")
  #texts_cyt.set_text("Ca")
  #texts_cytC.set_text("Cyosol")
  #angles = np.linspace(0, 2 * np.pi, len(c))
  #l.set_data(angles, c)
  #l1.set_data(angles, c1)
  vol_list, conc_fact = vol_dist()
  cyt_conc = np.asarray(cyt) * conc_fact * 1e3
  m_conc = np.asarray(m) * conc_fact * 1e3
  l2.set_data(x, y)
  #l21.set_data(x_vec, y)
  l3.set_data(x_vec,m_conc)
  l31.set_data(x_vec, cyt_conc)
  ...
  return l2,
"""    
circle1 = plt.Circle((2, 10), 0.5, fc = 'r', ec='r')
circle2 = plt.Circle((8, 10), 0.5, fc = 'r', ec='r')
plt.gca().add_patch(circle1)
...
#ani = animation.FuncAnimation(fig, update, frames = 500, interval = 1, blit = True)
#ani.save('shapeN3.mp4', fps=25, extra_args=['-vcodec', 'libx264'])
ani.save('shapeN3.mp4', writer = FFwriter)
#plt.show()
