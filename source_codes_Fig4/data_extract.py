...
fs = totalEnergy.fs
t_axis = np.linspace(0, tag.runtime, int(tag.runtime/c_dt))
...
class data_extract:
  ...
  def data_save_lists():
    ...
    location_list = tag.stim_locations
    ...
    for ne in range(1, 3):
        ...
        print(df_e)       
        df_e.to_csv("./energies" + str(ne) + ".csv")
    x_axis = np.linspace(0, len(phitot1_all) * c_dt * plot_interval, len(phitot1_all)) 
    ...
    for ns in range(1, 10):
     x_axis = np.linspace(0, len(globals()["phitot" + str(ns) + "_all"]) * c_dt * plot_interval, len(globals()["phitot" + str(ns) + "_all"])) 
    ...
    if conserv_check == True:   
       ...
       for c in check_files:
         ...
         if names[mol_name] == "Rac":
            ax11.plot( t_axis, normalize_exp_sim(max_an_y), 'r', label = names[mol_name] )
         if names[mol_name] == "Ca":
            ax10.plot( t_axis, normalize_exp_sim(np.asarray(max_an_y) - min(max_an_y)), 'b', label = names[mol_name] )
         if names[mol_name] == "thr286":
            ax9.plot( t_axis, normalize_exp_sim(max_an_y), 'b', label = names[mol_name] )
         mol_name = mol_name + 1
